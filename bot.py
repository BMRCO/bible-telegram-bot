...
def clean_text(text: str) -> str:
    ...
    # Espace français avant ? et ! uniquement (pas ; ni :)
...
def post_reel_to_facebook(video_path, ref, text, cat, cat_name):
    ...
    if r.status_code == 200:
        print(f"✅ Facebook reel publié — id: {r.json().get('id', 'inconnu')}")
    else:
...
def upload_video_public(video_path):
    ...
    with open(video_path, "rb") as f:
        r = requests.post(
            ...
            data={"api_key": CLOUDINARY_API_KEY, "timestamp": timestamp, "signature": signature, "resource_type": "video"},
            files={"file": f},
            timeout=180
        )
    if r.status_code == 200:
...
def post_to_instagram(image_path, ref, text, cat, cat_name):
    ...
    r = requests.post(container_url, data={"image_url": image_url, "caption": ig_caption, "access_token": FB_PAGE_TOKEN}, timeout=60)
    if r.status_code != 200:
    ...
    for attempt in range(8):
        ...
        rs = requests.get(status_url, params={"fields": "status_code", "access_token": FB_PAGE_TOKEN}, timeout=30)
    ...
    r2 = requests.post(publish_url, data={"creation_id": container_id, "access_token": FB_PAGE_TOKEN}, timeout=60)
...
def post_reel_to_instagram(video_path, ref, text, cat, cat_name):
    ...
    container_url = f"https://graph.facebook.com/v25.0/{IG_ACCOUNT_ID}/media"
    r = requests.post(container_url, data={"media_type": "REELS", "video_url": video_url, "caption": ig_caption, "access_token": FB_PAGE_TOKEN, "thumb_offset": "7500"}, timeout=60)
    if r.status_code != 200:
...
def make_cover_image(ref):
    ...
    draw = ImageDraw.Draw(img)
    draw.rounded_rectangle([BORDER, BORDER, W-BORDER, H-BORDER], radius=50, outline=blend(GOLD, 1.0), width=5)
    draw.rounded_rectangle([BORDER+12, BORDER+12, W-BORDER-12, H-BORDER-12], radius=42, outline=blend(GOLD, 0.3), width=1)
    try:
...
def pick_verse(progress):
    ...
    if cat_name is None:
        ...
        print(f"⚠️  Heure {hour_utc}h UTC hors créneau — fallback : {cat_name}")
    cat = CATEGORIES[cat_name]
    print(f"🕐 {hour_utc}h UTC → catégorie : {cat_name}")
